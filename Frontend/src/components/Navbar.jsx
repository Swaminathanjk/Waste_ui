  ...
  return (
    <nav className="bg-green-600 text-white p-4 shadow-lg fixed w-full top-0 z-50">
      <div className="container mx-auto flex justify-between items-center">
        <Link to="/" className="text-xl font-bold">Waste Sorter</Link>
        ...
        <div className="hidden lg:flex space-x-6">
          ...
          {user ? (
            <>
              <Link to="/profile" className="hover:text-gray-300">Hi!, {user.name}</Link>
              <button onClick={handleLogout} className="hover:text-gray-300">Logout</button>
            </>
          ) : (
            <>
              <Link to="/auth" className="hover:text-gray-300">Login</Link>
              <Link to="/auth" className="hover:text-gray-300">Signup</Link>
            </>
          )}
        </div>
      </div>
      ...
      {isOpen && (
        <div className="lg:hidden flex flex-col bg-green-700 p-4 absolute w-full left-0 top-16">
          ...
          <Link to="/leaderboard" className="py-2" onClick={() => setIsOpen(false)}>Leaderboard</Link>
          {user ? (
            <>
              <Link to="/profile" className="py-2" onClick={() => setIsOpen(false)}>Hi!, {user.name} </Link>
              <button onClick={handleLogout} className="py-2 text-left">Logout</button>
            </>
          ) : (
            <>
              <Link to="/auth" className="py-2" onClick={() => setIsOpen(false)}>Login</Link>
              <Link to="/auth" className="py-2" onClick={() => setIsOpen(false)}>Signup</Link>
            </>
          )}
        </div>
      )}
